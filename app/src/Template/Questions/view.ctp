...
<section class="question">
    <div class="card mb-3">
        <div class="card-body">
            <h5 class="card-title">
                <i class="fas fa-use-circle"></i> <?= 'たろう' // @TODO ユーザー管理機能実装時に修正する ?>
            </h5>
            <p class="card-text"><?= n12br(h($question->body)) ?></p>
            <p class="card-subtitle mb-2 text-muted">
                <small><i class="fas fa-comment-dots"></i> <?= $this->Number->format($answer->count()) ?></small>
            </p>
        </div>
    </div>
</section>
    ...
    <?php else: ?>
        ...
        <?php foreach ($answers as $answer): ?>
            <div class="card w-75 mb-2 ml-auto">
                <div class="card-body">
                    <h5 class="card-title">
                        <i class="fas fa-user-circle"></i> <?= 'じろう' // @TODO ユーザー管理機能実相寺に修正する ?>
                    </h5>
                    <p class="card-text"><?= n12br(h($answer->body)) ?></p>
                    <p class="card-subtitle mb-2 text-muted">
...
<section class="answer-post mb-5">
    ...
    <?php if ($answers->count() >= 100): ?>
        <p class="text-center">回答数が上限に達しているためこれ以上回答するkとはできません</p>
    <?php else: ?>
        ...
        echo $this->Form->hidden('questions_id', ['value' => $question->id]);
        ?>
